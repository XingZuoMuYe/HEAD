...
class SAC:
    ...
    def update(self, reward_scale=1., auto_entropy=True, target_entropy=-1, gamma=0.99, soft_tau=1e-2):
        ...
        reward = torch.FloatTensor(np.array(reward)).unsqueeze(dim=-1).to(self.device)
        done = torch.FloatTensor(np.float32(np.array(done))).unsqueeze(dim=-1).to(self.device)
        ...
        for target_param, param in zip(self.target_soft_q_net1.parameters(), self.soft_q_net1.parameters()):
            target_param.data.copy_(  # copy datasets value into target parameters
    ...
    def load(self, path):
        self.soft_q_net1.load_state_dict(torch.load(path + '/sac_soft_q1', weights_only=True))
        self.soft_q_optimizer1.load_state_dict(torch.load(path + "/sac_soft_q1_optimizer", weights_only=True))

        self.soft_q_net2.load_state_dict(torch.load(path + '/sac_soft_q2', weights_only=True))
        self.soft_q_optimizer2.load_state_dict(torch.load(path + "/sac_soft_q2_optimizer", weights_only=True))
        self.policy_net.load_state_dict(torch.load(path + "/sac_policy", weights_only=True))
        self.policy_optimizer.load_state_dict(torch.load(path + "/sac_policy_optimizer", weights_only=True))
