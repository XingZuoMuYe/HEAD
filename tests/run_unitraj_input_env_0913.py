...
object_type = defaultdict(lambda: default_value, object_type)
polyline_type = defaultdict(lambda: default_value, polyline_type)
SAVE_DIR = "/home/dw/HEAD/tests/figure"   # NEW
os.makedirs(SAVE_DIR, exist_ok=True)      # NEW
last_batch_dict = None                    # NEW
last_prediction = None                    # NEW
last_draw_index = 1                       # NEW 你也可以改成想看的样本索引
...
class UnitrajInference:
    ...
    def run_inference(self, num_iterations=20):
        """Run inference for a given number of iterations."""
        for i in range(num_iterations):
            ...
            batch_dict = to_device(batch_dict, device=self.device)
            t3 = time.time()

            prediction, loss = self.imitation_algo.forward(batch_dict)
            global last_batch_dict, last_prediction
            ...
            print(f"Iteration {i}:")
            print(f"Time1 (data processing): {t2 - t1:.4f}s")
            print(f"Time2 (agent preparation): {t3 - t2:.4f}s")
            print(f"Time3 (inference): {t4 - t3:.4f}s")
...
if __name__ == '__main__':
    ...
    inference_engine.run_inference(num_iterations=20)
    # ====== for 循环结束后，保存一张带时间戳的对比图 ======
    if (last_batch_dict is not None) and (last_prediction is not None):  # NEW
        ts = datetime.now().strftime("%Y%m%d-%H%M%S-%f")  # NEW
        final_path = os.path.join(SAVE_DIR, f"prediction_vs_gt_{ts}.jpg")  # NEW
        visualization.visualize_prediction(  # NEW
            last_batch_dict, last_prediction,
            draw_index=last_draw_index,
            save_path=final_path,
            rotate=180
        )
        print("✅ 已保存：", final_path)  # NEW
