...
from head.agents import AgentInput, create_agent
...
def test_real_weights_predict_bundled_scenarios(model):
    ...
    agent = create_agent(OmegaConf.create({"model": model}), checkpoint, device="cpu")
    ...
    for dataset in ["waymo", "geely"]:
        ...
        for step in steps:
            result = agent.compute_trajectory(AgentInput(scenario, step))
...
def test_declared_future_context_scope(model):
    ...
    with zipfile.ZipFile(root / "head/scenario_datasets/waymo.zip") as archive:
        ...
        scenario = pickle.loads(archive.read(member))
    before = agent.compute_trajectory(AgentInput(copy.deepcopy(scenario), 21)).samples
