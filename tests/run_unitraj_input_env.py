...
if __name__ == '__main__':
    # 获取配置
    cfg = get_final_config()  # 外层配置，包含 method=wayformer

    imitation_algo, global_cfg = resolve_imitation_strategy(cfg)

    env = make_env(cfg)
    ckpt = torch.load(global_cfg.ckpt_path, map_location = "cuda", weights_only=False)
    imitation_algo.load_state_dict(ckpt["state_dict"])
    imitation_algo = imitation_algo.to("cuda")
    for i in range(20):
        t1 = time.time()
        scenario_data = env.engine.data_manager._scenarios
        ...
        past_length = global_cfg['past_len']
        future_length = global_cfg['future_len']

        total_steps = past_length + future_length
        starting_fame = 0
        ending_fame = starting_fame + total_steps
        trajectory_sample_interval = global_cfg['trajectory_sample_interval']
        ...
        ret['current_time_index'] = global_cfg['past_len'] - 1
        ret['sdc_track_index'] = track_infos['object_id'].index(ret['sdc_id'])

        if global_cfg['only_train_on_ego']:
            tracks_to_predict = {
        ...
        center_objects, track_index_to_predict = get_interested_agents( global_cfg,
        ...
        (obj_trajs_data, obj_trajs_mask, obj_trajs_pos, obj_trajs_last_pos, obj_trajs_future_state,
         ...
         track_index_to_predict_new) = get_agent_data(global_cfg,
        ...
        if global_cfg.manually_split_lane:
            map_polylines_data, map_polylines_mask, map_polylines_center = get_manually_split_map_data(global_cfg,
                center_objects=center_objects, map_infos=info['map_infos'])
        else:
            map_polylines_data, map_polylines_mask, map_polylines_center =  get_map_data(global_cfg,
                center_objects=center_objects, map_infos=info['map_infos'])
        ...
        masked_attributes = global_cfg['masked_attributes']
        ...
        batch_dict = to_device(batch_dict, device="cuda")
        t3 = time.time()
        prediction, loss  = imitation_algo.forward(batch_dict)


        #
        # img = visualization.visualize_prediction(batch_dict, prediction)
        # #
        # img.show()
