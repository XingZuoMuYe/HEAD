...
class TrajectoryEvaluator:
    ...
    def _resolve_debug_target(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        candidate_env_paths = []
        candidate_env_paths.append(os.path.join(os.getcwd(), ".dbg", "pluto-turning-drift.env"))

        cur_dir = os.path.dirname(os.path.abspath(__file__))
        while True:
            candidate_env_paths.append(os.path.join(cur_dir, ".dbg", "pluto-turning-drift.env"))
            ...
            try:
                with open(env_path, "r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        if key == "DEBUG_SERVER_URL" and not server_url:
                            server_url = value
                        elif key == "DEBUG_SESSION_ID" and not session_id:
                            session_id = value
                if server_url and session_id:
                    break
            except Exception:
    ...
    def _emit_debug_log(self, payload: Dict) -> None:
        server_url, session_id = self._resolve_debug_target()
        ...
        try:
            body = {
                ...
                "runId": "pre",
                "hypothesisId": "K",
                "location": "unitraj/closeloop/pluto/trajectory_evaluator.py:evaluate",
                "msg": "[DEBUG] evaluator component breakdown",
                "data": payload,
            }
