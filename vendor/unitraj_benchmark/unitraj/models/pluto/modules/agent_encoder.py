...
class AgentEncoder(nn.Module):
    ...
    def forward(self, data):
        ...
        valid_mask = data["agent"]["valid_mask"][:, :, :T].bool()
        ...
        x_agent = torch.zeros(bs * A, self.dim, device=position.device, dtype=position.dtype)
        # 确保dtype一致（处理混合精度训练）
        x_agent[valid_agent_mask] = x_agent_tmp.to(x_agent.dtype)
