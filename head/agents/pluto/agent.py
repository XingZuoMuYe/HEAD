...
from head.agents.base import BaseAgent, Trajectory
...
class Agent(BaseAgent):
    ...
    def compute_trajectory(self, agent_input):
        if getattr(self, "sae_experiment", None) is not None:
            self.sae_experiment.context = (str(agent_input.scenario["id"]), agent_input.current_step)
        scenario = normalize_scalar_states(agent_input.scenario)
        output = self.engine.run_inference(scenario, agent_input.current_step)
        ...
        if trajectory.shape[1] >= 6:
            ...
            samples = trajectory
        return Trajectory(samples, self.dt, self.reference_offset(agent_input.scenario, agent_input.current_step))
