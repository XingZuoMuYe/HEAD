...
# Standalone inference module: no Lightning evaluator dependency.
from .layers import PerceiverEncoder, PerceiverDecoder, TrainableQueryProvider
...
class Wayformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.closeloop = config.get("closeloop_mode", True)
        self.config = config
    ...
    def _forward(self, inputs):
        ...
        if not torch.isfinite(out_dists).all():
            raise ValueError("WayFormer produced non-finite trajectories")
        return output
