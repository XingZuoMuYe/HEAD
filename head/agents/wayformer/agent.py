...
class Agent(BaseAgent):
    ...
    def initialize(self, checkpoint):
        if self.config.get("sae", {}).get("mode", "off") not in ("off", False):
            raise ValueError("This WayFormer agent does not support the Pluto SAE hook")
    ...
    def compute_trajectory(self, agent_input):
        ...
        batch, centres = self.dataset.process_scenario(scenario, agent_input.current_step)
