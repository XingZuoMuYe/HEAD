...
class FrontBackObjects:
    ...
    @classmethod
    def find_lane(cls, centers, lane_width, pedestrian_y):
        # 计算每个车道的范围
        lanes = [(center - lane_width / 2, center + lane_width / 2) for center in centers]

        # 检查行人的横向坐标在哪个车道的范围内
        for i, (lower_bound, upper_bound) in enumerate(lanes):
            if lower_bound <= pedestrian_y <= upper_bound:
                return i
    ...
    def get_find_front_back_objs(cls, objs, lane, position, max_distance, ref_lanes=None):
        ...
        for i, lane in enumerate(lanes):
            if lane is None:
                continue
            for obj in objs:
                if obj.metadrive_type == 'PEDESTRIAN':
                    lat_centers = []
                    lane_width = ref_lanes[0].width
                    for j in range(len(ref_lanes)):
                        lat_centers.append(ref_lanes[j].get_polyline()[0][1])

                    lane_id = cls.find_lane(lat_centers, lane_width, obj.position[1])
                    if lane_id != -1:
                        obj.lane = ref_lanes[lane_id]
                if obj.lane is lane:
...
class IDMPolicyIncluedPedestrain(BasePolicy):
    """
    ...
    def __init__(self, control_object, random_seed):
        super(IDMPolicyIncluedPedestrain, self).__init__(control_object=control_object, random_seed=random_seed)
...
class ManualControllableIDMPolicy(IDMPolicyIncluedPedestrain):
...
class TrajectoryIDMPolicy(IDMPolicyIncluedPedestrain):
