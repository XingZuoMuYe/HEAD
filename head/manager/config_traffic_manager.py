...
class NaturalTrafficManager(BaseManager):
    VEHICLE_GAP = 10  # m
    """
    NaturalTrafficManager: 适用于 OSM 地图导入的交通流管理器，不依赖轨迹数据，在地图上动态生成、移除车辆，主要用于自然交通流仿真
    ...
    def __init__(self):
        ...
        控制整个交通流
        """
        super(NaturalTrafficManager, self).__init__()
    ...
    def before_reset(self) -> None:
        ...
        :return: None
        """
        super(NaturalTrafficManager, self).before_reset()
        self.density = 0.1
        self.block_triggered_vehicles = []
        self._traffic_vehicles = []
    ...
    def _create_respawn_vehicles(self, map: BaseMap, traffic_density: float, near_ego_lane: bool = False):
        ...
        for traffic_v_config in selected:
            ...
            # todo：周车加入MyPolicy。自车：_create_agents Vehicle_type选 BaseVehicle 的子类 → 生成车辆对象 → 分配驾驶策略
            self._traffic_vehicles.append(random_v)
    ...
    @property
    def vehicles(self):
        return list(self.engine.get_objects(filter=lambda o: isinstance(o, BaseVehicle)).values())  # todo: revised BaseVehicle
    ...
    def seed(self, random_seed):
        if not self.random_traffic:
            super(NaturalTrafficManager, self).seed(random_seed)
    ...
    def get_state(self):
        ret = super(NaturalTrafficManager, self).get_state()
        ret["_traffic_vehicles"] = [v.name for v in self._traffic_vehicles]
        flat = []
        for b_v in self.block_triggered_vehicles:
            flat.append((b_v.trigger_road.start_node, b_v.trigger_road.end_node, b_v.vehicles))
    ...
    def set_state(self, state: dict, old_name_to_current=None):
        super(NaturalTrafficManager, self).set_state(state, old_name_to_current)
        self._traffic_vehicles = list(
            self.get_objects([old_name_to_current[name] for name in state["_traffic_vehicles"]]).values()
        )
        self.block_triggered_vehicles = [
