...
def parse_cfg(cfg_path) -> OmegaConf:
	...
	for k,v in cli.items():
		if v == None:
			cli[k] = True
	if 'profile' in cli:
		raise ValueError("'profile' has been replaced by the locked 'workflow' selector")
	base.merge_with(cli)

	# Modality config
	if cli.get('modality', base.modality) not in {'state'}:
		raise ValueError("Invalid modality: only 'state' is currently supported")
