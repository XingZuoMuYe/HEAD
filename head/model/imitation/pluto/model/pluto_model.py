...
class PlanningModel(nn.Module):
    def __init__(
        ...
        self.use_hidden_proj = config.get('use_hidden_proj')
        self.num_modes = config.get('num_modes')
        self.radius = config.get('radius')
        self.ref_free_traj = config.get('ref_free_traj')
    ...
    def forward(self, data):
        ...
        if not self.training:
            ...
            if trajectory is not None:
                ...
                probability_masked = probability.masked_fill(r_padding_mask.unsqueeze(-1), -1e6)
                ...
                flattened_probability = probability_masked.reshape(bs, R * M)
