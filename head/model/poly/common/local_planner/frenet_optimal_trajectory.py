...
from head.model.poly.common.local_planner import cubic_spline_planner

from head.model.poly.common.config import cfg
from head.model.poly.common.utils import calc_cur_s, EGO_POSE
...
class FrenetPlanner:
    ...
    def estimate_frenet_state_new(self, ego_state, idx):
        ...
        cur_s, f_idx = calc_cur_s(self.csp, ego_pose, max(idx - 2, 0))

        estimated_s = cur_s % ego_state[6]
        ...
        if self.last_fpath is None:
            ...
            dis_mux = []
            for i in range(len(self.last_fpath.x)):
                dis = (ego_state[0] - self.last_fpath.x[i]) ** 2 + (ego_state[1] - self.last_fpath.y[i]) ** 2
                dis_mux.append(dis)
            min_index = dis_mux.index(min(dis_mux))
            estimated_s = self.last_fpath.s[min_index]
            d = self.last_fpath.d[min_index]
            s_d = self.last_fpath.s_d[min_index]
            d_d = self.last_fpath.d_d[min_index]

        f_state[0] = estimated_s  # if estimated_s != None else 0
        f_state[2] = ego_state[4] - s_yaw
    ...
    def reset(self, s, d, s_d=0, s_dd=0, d_d=0, d_dd=0, df_n=0, Tf=4, Vf_n=0, optimal_path=True):
        ...
        self.last_f_state = [s, s_d, s_dd, d, d_d, d_dd]
        if optimal_path:
...
class FrenetPlanner_safe:
    ...
    def run_step_single_path(self, ego_state, idx, df_n=0, Tf=4, Vf_n=0):
        ...
        if self.steps == 1:
            self.path = self.generate_single_frenet_path(f_state, df=df_n, Tf=Tf, Vf=Vf)
            self.last_f_state = f_state
        else:
            self.path = self.generate_single_frenet_path(self.last_f_state, df=df_n, Tf=Tf, Vf=Vf)

            for i in range(len(self.path.s) - 1):
                if self.path.s[i] < f_state[0] < self.path.s[i + 1]:
                    break
            self.last_f_state = [self.path.s[i], self.path.s_d[i], self.path.s_dd[i],
                                 self.path.d[i], self.path.d_d[i], self.path.d_dd[i]]
